package xyz.arcadiadevs.gensforge.statics;
...
/**
 * The Messages class contains all the messages used in GensForge.
 */
public class Messages {
  ...
  public static void init() {
    final File file = new File(GensForge.getInstance().getDataFolder(), "messages.yml");
    ...
    CONFIG_RELOADED = config.getString(
        "config-reloaded",
        "&9GensForge> &7Configuration reloaded."
    );
    ...
    GENERATOR_GIVEN = config.getString(
        "generator-given",
        "&9GensForge> &7You gave &a%amount% &7generator(s)"
            + " of tier &a%tier% &7to &a%targetPlayer%"
    );

    GENERATOR_RECEIVED = config.getString(
        "generator-received",
        "&9GensForge> &7You received &a%amount% &7generator(s) of tier &a%tier%"
    );
    ...
    EVENT_STARTED = config.getString(
        "event-started",
        "&9GensForge> &7%event% has started and will end in &e&n%time%!"
    );

    EVENT_ENDED = config.getString(
        "event-ended",
        "&9GensForge> &7%event% has ended and new event will be started in &e&n%time%!"
    );

    SUCCESSFULLY_UPGRADED = config.getString(
        "successfully-upgraded",
        "&9GensForge> &7Successfully upgraded your generator to tier &a%tier%!"
    );

    SUCCESSFULLY_SOLD = config.getString(
        "successfully-sold",
        "&9GensForge> &7Successfully sold drops for &a%price%"
    );
    ...
    SUCCESSFULLY_DESTROYED = config.getString(
        ...
        "&9GensForge> &7Successfully destroyed generator!"
    );

    SUCCESSFULLY_PLACED = config.getString(
        "successfully-placed",
        "&9GensForge> &7Successfully placed generator tier %tier%!"
    );

    SUCCESSFULLY_BOUGHT = config.getString(
        "successfully-bought",
        "&9GensForge> &7Successfully bought generator tier %tier% for %price%!"
    );
    ...
    GENERATOR_GIVEN_ALL = config.getString(
        "generator-given-all",
        "&9GensForge> &7You gave &a%amount% &7generator"
            + " of tier &a%tier% &7to all players! (&a%count%&7)"
    );

    DEFAULT_MESSAGE = config.getString(
        "default-message",
        "&9GensForge> &7This server is running GensForge &av%version%"
    );
