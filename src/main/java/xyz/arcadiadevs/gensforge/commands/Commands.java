...
import xyz.arcadiadevs.gensforge.guis.GeneratorsGui;
...
import xyz.arcadiadevs.gensforge.utils.SellUtil;

/**
 * The Commands class implements the CommandExecutor interface to handle custom commands in
 * GensForge. It provides functionality for various commands related to generators and the
 * plugin itself.
 */
public class Commands implements CommandExecutor {
  ...
  /**
   * Constructs a Commands object with the specified GensForge instance and GeneratorsData.
   *
    ...
    if (command.getName().equalsIgnoreCase("gensforge")) {
      if (strings.length == 0) {
        ChatUtil.sendMessage(player, Messages.DEFAULT_MESSAGE.replace("%version%",
            GensForge.getInstance().getDescription().getVersion()));
        return true;
      }
      ...
      if (strings[0].equalsIgnoreCase("help")) {
        ChatUtil.sendMessage(player,
            "&9GensForge Commands:");
        ChatUtil.sendMessage(player,
            "&7- /gensforge: Display plugin version");
        ChatUtil.sendMessage(player,
            "&7- /gensforge give <player> <tier> [amount]: Give a generator to a player");
        ChatUtil.sendMessage(player,
            "&7- /gensforge giveall <tier> [amount]: Give a generator to all players");
        ChatUtil.sendMessage(player,
      ...
      /*if (strings[0].equalsIgnoreCase("reload")) {
        ...
        GensForge.getInstance().reloadConfig();
