...
import xyz.arcadiadevs.gensforge.utils.HologramsUtil;
...
public record LocationsData(CopyOnWriteArrayList<GeneratorLocation> locations) {
  ...
  public static class GeneratorLocation {
    ...
    public GeneratorLocation(String playerId, Integer generator, List<?> blockLocations) {
      ...
      List<Map<?, ?>> generatorsConfig = GensForge.getInstance()
    ...
    public void spawn() {
      ...
      final GensForge instance = GensForge.getInstance();
      ...
      final long ticks = TimeUtil.parseTime(
          GensForge.getInstance().getConfig().getString("item-despawn-time"));

      Bukkit.getScheduler().runTaskLater(GensForge.getInstance(), () -> {
